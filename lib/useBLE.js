...
const BATTERY_CHARACTERISTIC_UUID = "12345678-1234-5678-1234-56789abcdef4";
const CMD_CHARACTERISTIC_UUID = "12345678-1234-5678-1234-56789abcdef2";
const RESP_CHARACTERISTIC_UUID = "12345678-1234-5678-1234-56789abcdef3";
...
function useBLE() {
  ...
  const wasTaredInitial = useRef(false);
        ...
        subscription = device.monitorCharacteristicForService(
            ...
            (error, char) => {
                ...
                if (char?.value) {
                    ...
                    // Check for the success byte '1' (0x31)
                    if (decoded === '1') {
...
const resetWeightState = () => {
  ...
  const updateAndDetectFill = (rawWeight) => {
    // Show raw weight instantly, no lag
    ...
    if (isStable(3000)) {
        const delta = avg - lastStableWeight.current;

        if (Math.abs(delta) > FILL_THRESHOLD) {
            currentWeight.current = avg;

            if (delta > 0) {
                if (avg > maxWeight.current) {
                    maxWeight.current = avg;
                    console.log("Fill detected | maxWeight:", maxWeight.current);
                }
            } else {
                waterRunningTotal.current += Math.abs(delta);
                setWater(waterRunningTotal.current);
                console.log("Drink detected | delta:", delta, "| total:", waterRunningTotal.current);
            }
  ...
  const onDataUpdate = (error, characteristic) => {
    ...
    if(characteristic.uuid == WEIGHT_CHARACTERISTIC_UUID){
      ...
      //   tare();
      // }
      console.log("Raw weight: ", wt);
      if(wt >= 0 || Math.abs(wt) < 1){
        if(Math.abs(wt) < 1){
          updateAndDetectFill(Math.abs(wt));
        }
        else{
    ...
    if(characteristic.uuid == BATTERY_CHARACTERISTIC_UUID){
      ...
      setBattery(rawBat.readFloatLE(0));
    }


    //response
    //  if(characteristic.uuid == RESP_CHARACTERISTIC_UUID){
    //   const rawBat = Buffer.from(characteristic.value, 'base64');
    //   setBattery(rawBat.readFloatLE(0));
    // }
